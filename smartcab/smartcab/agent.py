...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=False, epsilon=0.95, alpha=0.1):
    ...
    def build_state(self):
        ...
        States:
            is_red_light: Whether the traffic light is red. (red: True, green: False)
            oncoming: Whether there is an oncoming car . (yes: True, no: False)
        """


        if self.learning:
            is_red_light =  True if inputs['light']  == 'red' else False
            oncoming = True if inputs['oncoming'] else False
            state = (is_red_light, oncoming, waypoint)
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            ...
            if draw == 'learning': 
                ...
                max_action = [] # list of actions that have the highest Q value
                for key, value in self.Q[state].iteritems():
                    if value == max_Q:
                        max_action.append(key)
                action = random.choice(max_action) # randomly choose a feasible action.
